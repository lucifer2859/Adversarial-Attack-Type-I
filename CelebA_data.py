...
def split_and_generate_npy(dataset_root_dir='/home/dchen/dataset/CelebA/', output_root_dir='/home/dchen/dataset/CelebA/GenderSplit'):
...
def generate_npy(dataset_root_dir='/home/dchen/dataset/CelebA/', output_root_dir='/home/dchen/dataset/CelebA/AttrSplit'):
    list_eval_partition_path = os.path.join(dataset_root_dir, 'Eval/list_eval_partition.txt')
    list_attr_path = os.path.join(dataset_root_dir, 'Anno/list_attr_celeba.txt')
    img_dir = os.path.join(dataset_root_dir, 'Img/img_align_celeba/')
    ...
    for idx, partition in enumerate(partitions):
        ...
        for ind in range(len(attr_list)):
            attr_flag = int(attr[ind + 1])
            label.append((0 if attr_flag == -1 else 1))
...
def load_celebA_Gender(data_dir='/home/dchen/dataset/CelebA/GenderSplit', onehot=False, test_only=False, prep=True):
    if not test_only:
...
def load_celebA_Attr(data_dir='/home/dchen/dataset/CelebA/AttrSplit', test_only=False, prep=True):
    if not test_only:
        x_train = np.load(os.path.join(data_dir, 'train_imgs.npy'))
        y_train = np.load(os.path.join(data_dir, 'train_labels.npy'))
    else:
