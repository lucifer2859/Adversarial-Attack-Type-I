...
class SampleNet(nn.Module):
    def __init__(self, downSample, in_channels, out_channels, device, kernel_size=2, stride=2, padding=0, groups=1, step=2):
        ...
        if (self.downSample == True):
            ...
            self.gdn = pytorch_gdn.GDN(out_channels, device=device, inverse=False)

        elif (self.downSample == False):

            self.igdn = pytorch_gdn.GDN(in_channels, device=device, inverse=True)
            if (self.step == 2):
